...
| **API origin**            | Same-origin: `vercel.json` rewrites проксируют `/api/*` → backend (`http://194.32.140.219:5678`). Браузер ↔ Vercel — HTTPS; Vercel ↔ backend — server-side (нет mixed-content при HTTP-бэкенде). CORS не нужен.                                                                                                                                                                 |
