...
**Контракты сверены с live Swagger** (`$BACKEND_ORIGIN/docs-json`, последний аудит: 2026-05-14, OpenAPI ≈ 383KB, 202 paths из них 13 под `/saas/*` + 2 для super-admin под `/admin/*` + 2 health). Все URL/DTO/response shape'ы в этом плане — точные. Если backend меняется — `pnpm gen:api` + ре-аудит.
...
| Что                               | URL                         |
| --------------------------------- | --------------------------- |
| Backend base                      | `$BACKEND_ORIGIN`           |
| API prefix (для всех endpoints)   | `/api/v1/`                  |
| Swagger UI                        | `$BACKEND_ORIGIN/docs`      |
| Swagger JSON (для `pnpm gen:api`) | `$BACKEND_ORIGIN/docs-json` |
...
server: {
  proxy: {
    '/api': { target: process.env.BACKEND_ORIGIN, changeOrigin: true },
  },
}
```
...
```bash
# Backend health
curl -sS "$BACKEND_ORIGIN/api/v1/health" | head
...
curl -sS "$BACKEND_ORIGIN/docs-json" | wc -c
# → должно быть > 100000 (~383KB)

# SuperAdmin login работает
curl -sS -X POST "$BACKEND_ORIGIN/api/v1/saas/auth/login" \
...
1. `vercel.json` — framework `vite`, `pnpm install --frozen-lockfile` / `pnpm build` / `outputDirectory: dist`; Routing Middleware проксирует `/api/:path*` → `BACKEND_ORIGIN`, SPA fallback `/(.*)` → `/index.html`; security headers (CSP/HSTS/nosniff/X-Frame-Options/Referrer-Policy/X-Robots-Tag) + cache (`/assets/*` immutable, `index.html` no-store).
...
- Backend origin настраивается через `BACKEND_ORIGIN` и Routing Middleware, см. `architecture.md §11`.
